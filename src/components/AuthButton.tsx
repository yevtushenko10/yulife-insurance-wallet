...
export function AuthButton({ user }: AuthButtonProps) {
  ...
  if (user) {
    return (
      <button
        ...
        className="flex items-center gap-2 bg-white/80 border border-gray-100 rounded-2xl px-3 py-1.5 shadow-sm"
      >
        {user.photoURL && (
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-2xl px-3 py-1.5 shadow-sm hover:bg-gray-50 transition-colors"
    >
      ...
      <span className="text-xs font-bold text-gray-600">Sign in</span>
    </button>
  );
}
